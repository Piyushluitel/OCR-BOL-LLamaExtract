...
if not st.session_state.authenticated:
    ...
    # Option 2: Select a file from the local filenames or manually enter the filename
    elif option == "Select from s3 bucket":
        # Provide a text input to allow user to manually enter the filename from S3
        entered_filename = st.text_input("Enter the filename from S3 bucket (e.g., 'folder/filename.jpg'):")

        if entered_filename:
            # Try to fetch the file from S3 using the entered filename
            presigned_url = get_s3_presigned_url('fp-prod-s3', entered_filename)
            ...
            if presigned_url:
                try:
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(entered_filename)[1]) as tmp_file:
                    ...
                    with col1:
                        ...
                        st.image(image, caption=f"Uploaded Document from S3: {entered_filename}", use_container_width=True)
